...
TriggerEvent('chat:addSuggestion', '/drone', 'Crie ou exclua um drone controlado remotamente')
    ...
    while (true) do
        if (drone.active) then
            ...
            if (distance <= 1000.0) then
                ...
                showNotification("Você ~b~perdeu o controle ~w~do drone, depois de sair do alcance")
                deleteDrone()
            end
...
function setupScaleform(scaleform)
    ...
    ButtonMessage("Vire à direita")
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(2)
    Button(GetControlInstructionalButton(0, 174, true)) -- O botão para exibir
    ButtonMessage("Vire à esquerda") -- a mensagem a ser exibida ao lado dela
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(3)
    Button(GetControlInstructionalButton(0, 173, true))
    ButtonMessage("Para trás")
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(0, 172, true))
    ButtonMessage("Para frente")
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(5)
    Button(GetControlInstructionalButton(0, 18, true))
    ButtonMessage("Camera")
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(6)
    Button(GetControlInstructionalButton(0, 212, true))
    ButtonMessage("Visão Noturna")
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(7)
    Button(GetControlInstructionalButton(0, 214, true))
    ButtonMessage("Imagens Térmicas")
    PopScaleformMovieFunctionVoid()
...
function newDrone()
    ...
    drone.active = true
    showNotification("Drone ~b~COLOCADO~w~.")
end
...
function deleteDrone()
    ...
    showNotification("Drone ~b~REMOVIDO~w~.")
end
