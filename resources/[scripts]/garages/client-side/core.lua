...
function Creative.SpawnPosition(Select)
    ...
    repeat
        ...
        if Garages[Select] and Garages[Select][Slot] then
            local _, CoordsZ = GetGroundZFor_3dCoord(Garages[Select][Slot][1], Garages[Select][Slot][2],
                Garages[Select][Slot][3])
            Selected = { Garages[Select][Slot][1], Garages[Select][Slot][2], CoordsZ, Garages[Select]
                [Slot][4] }
    ...
    if not Garages[Select][tostring(Checks)] then
        TriggerEvent("Notify","Aviso","Vagas estão ocupadas.","amarelo",5000)
        return false
    end
...
AddEventHandler("garages:Delete", function(Vehicle)
    ...
    if IsEntityAVehicle(Vehicle) and (not Entity(Vehicle)["state"]["Tow"] or LocalPlayer["state"]["Admin"]) then
		...
		vSERVER.Delete(VehToNet(Vehicle), GetEntityHealth(Vehicle), GetVehicleEngineHealth(Vehicle),GetVehicleBodyHealth(Vehicle), Entity(Vehicle)["state"]["Fuel"], Doors, Windows, Tyres,GetVehicleNumberPlateText(Vehicle))
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- SEARCHBLIP
...
AddEventHandler("garages:Impound", function()
    ...
    if parseInt(#Impound) > 0 then
        ...
        TriggerEvent("Notify","Aviso","Você não possui veículos apreendidos.","amarelo",5000)
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADOPEN
