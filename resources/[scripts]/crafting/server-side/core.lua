...
function Creative.Take(Item,Amount,Target,Name)
	...
	if Passport and Item and Target and List[Name] and List[Name]["List"] and List[Name]["List"][Item] then
		...
		if not vRP.MaxItens(Passport,Item,Multiplier) and vRP.InventoryWeight(Passport,Item,Multiplier) and (not Inventory[Target] or (Inventory[Target] and Inventory[Target]["item"] == Item)) then
