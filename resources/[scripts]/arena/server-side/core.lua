...
function Creative.CheckEnter(Route,Number)
	...
	if Passport then
		if Arenas[Number] then
			if vRP.Request(source,"Prosseguir para a <b>Arena</b> pagando <b>$"..Dotted(Arenas[Number]["Price"]).."</b> dólares?","Sim, por favor","Não, volto mais tarde") then
				if vRP.PaymentBank(Passport,Arenas[Number]["Price"]) then
					...
					TriggerClientEvent("Notify",source,"Negado","<b>Dólares</b> insuficientes.","vermelho",5000)
				end
			end
		else
			vRP.SaveTemporary(Passport,source,Route)
