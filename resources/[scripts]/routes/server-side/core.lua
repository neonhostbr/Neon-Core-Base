...
-- ROUTES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("routes",function(source,Message)
    local Passport = vRP.Passport(source)
    if vRP.HasGroup(Passport,"Admin") and Message[1] then
        if not Config[Message[1]] then
            TriggerClientEvent('Notify',source,'aviso',string.format('Nome inválido: %s', Message[1]))
            return
        end
        SetEntityCoords(GetPlayerPed(source), Config[Message[1]]['Init'])
        TriggerClientEvent('routes:Open',source,Message[1])
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- REWARD
