...
AddEventHandler("inventory:RobberySingle",function(Number,Mode)
    ...
    if Passport and not Active[Passport] and Config[Mode] and not Config[Mode]["Active"] then
        ...
        local policeCount = vRP.AmountService("Policia") or 0

        if policeRequirement > 0 and policeCount < policeRequirement then
        ...
        if cooldown <= os.time() then
            ...
            if Config[Mode]["Last"] == Number then
                ...
                TriggerClientEvent("Notify", source, "Atenção", "Aguarde "..CompleteTimers(cooldown - os.time())..".","amarelo",5000)
            end
        end
    end
end)
