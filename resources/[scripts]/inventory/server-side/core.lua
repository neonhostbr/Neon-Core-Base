...
function Creative.Blueprint()
	...
	if Passport and Users["Blueprints"][Passport] then
		...
		return Primary,vRP.CheckWeight(Passport),Secondary
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- SEND
...
function Creative.PreventWeapons(Item,Ammo)
	...
	if Passport and Users["Ammos"][Passport] then
		...
		if ItemType(Item,"Armamento") and vRP.InventoryItemAmount(Passport,Item) then
			return true
		end
	end
					...
					if exports["party"]:DoesExist(Passport,2) then
						...
						for Number = 1,AmountMembers do
							if vRP.Passport(Consult[Number]["Source"]) and vRPC.LastVehicle(Consult[Number]["Source"],"trash") then
								if not vRP.MaxItens(Consult[Number]["Passport"],Result["Item"],Valuation) and vRP.InventoryWeight(Consult[Number]["Passport"],Result["Item"],Valuation) then
	...
	if Passport and not Active[Passport] and Entity[5] then
		...
		if MyEntity == Entity[1] then
			if vCLIENT.CheckWeapon(source,"WEAPON_SWITCHBLADE") then
				if vRP.InventoryWeight(Passport,"deer1star") then
