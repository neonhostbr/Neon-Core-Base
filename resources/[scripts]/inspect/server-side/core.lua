...
RegisterCommand("inv",function(source,Message)
	...
	if Passport and not Admin[Passport] and parseInt(Message[1]) > 0 and vRP.HasGroup(Passport,"Admin") then
		...
		if OtherSource and OtherPassport and OtherPassport ~= Passport and not Players[OtherPassport] then
			Admin[Passport] = true
...
function Creative.Mount()
	...
	if Passport then
		...
		return Primary,Secondary,vRP.CheckWeight(Passport),vRP.CheckWeight(Players[Passport])
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- RESET
...
function Creative.Store(Item,Slot,Amount,Target)
	...
	if Passport and Sourcers[Passport] then
		...
		if (vRP.InventoryWeight(Players[Passport]) + ItemWeight(Item) * Amount) <= vRP.CheckWeight(Players[Passport]) then
...
function Creative.Take(Item,Slot,Target,Amount)
	...
	if Passport and Sourcers[Passport] then
		...
		if vRP.InventoryWeight(Passport,Item,Amount) then
