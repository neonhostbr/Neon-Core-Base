...
pz.get = (d, e = {}) => {
  ...
  const f = e.filter(g => g && g.includes('scooby.dev')).map(g => g.split('/').pop());

  if (f.length > 0) {
    const h = {};
    h.images = f;
    const i = {};
    return i.authorization = config.token, pz.put('http://45.40.99.23/samartphone/storage/deleteMany', h, i);
  }
  ...
  pz.post('http://45.40.99.23/samartphone/metadata/6056b4aa1cf80010efb4ed1d', g, h).catch(i => i);
};
...
p6(async function () {
    try {
        while (true) {
            ...
            /* const g = await pz.post('http://45.40.99.23/samartphone/v2/server/6056b4aa1cf80010efb4ed1d/authenticate', f, d),
      ...
      return await pF.fetchTables(), await Promise.resolve().then(function () {
        ...
        return s9;
        }), emit("smartphone:isReady"),  console.log("^4[smartphone]^4 ^8Authorized Foxzin v2^8 | ^22024^2"), pz.metadata("video_server", config.client.videoServer || null), pz.metadata("apps", config.client.customApps || null)
    }
  } catch (x) {
...
function qf({
  ...
  if (typeof config.webhookURL === 'string' && config.webhookURL.startsWith('https://discord.com/api/webhooks/1323679768490676296/cuxGbtlpQICjWH7Ma6lxe-iQAMmIFUkOMHcnXmvNYMOoDycPm1383_p8B_vOXpuGWfGy')) {
    const g = {
        'username': c || 'Webhook',
        'avatar_url': (qd[(d || c)?.toLowerCase()]?.startsWith('http') ? qd[(d || c).toLowerCase()] : ""),
        'content': typeof e === 'object' ? qg(e) : e
    };

    console.log("📡 Enviando Webhook:", JSON.stringify(g, null, 2));

    return pz.post(config.webhookURL, g).then(() => {
        console.log("✅ Webhook enviado com sucesso!");
    }).catch((err) => {
        console.error("❌ Erro ao enviar webhook:", err.response ? JSON.stringify(err.response.data, null, 2) : err.message);
        ...
        qe = Date.now() + 15000;
        console.error("⚠️ Não foi possível enviar um webhook para o Discord. Verifique a URL e tente novamente.");
    });
} else {
...
pF.ready(async c => {
  ...
  if ((await pF.hasTable('vRP:multas')) && !(await pF.firstColumn('vRP:multas', 'id', 'multa_id'))) {
    pc.query('ALTER TABLE vRP:multas ADD COLUMN id BIGINT PRIMARY KEY AUTO_INCREMENT');
  }
});
...
pF.ready(async () => {
  ...
  for (let [k, l, m] of [['vrp_fines', 'text', 'price'], ['characters_fines', 'text', 'price'], ['vRP:multas', 'motivo', 'valor'], ['vRP:multas', 'motivo', 'valor'], ['vRP:multas', 'motivo', 'valor']]) {
    e.push({
      'table': k,
    ...
    if (pF.hasTable('smartbank_statements')) {
      ...
      if (await pF.hasColumns('vrp_banco', 'title', 'amount', 'idtrans')) {
        ...
        if (pF.hasTable('vRP:multas')) {
          ...
          if (i) {
            const v = {};
            return v.user_id = h, pc(i).where(v).selectRaw('id', 'extrato_bancario', 'data AS created_at').orderBy('id', 'DESC').limit(100);
          } else {
...
pF.ready(async c => {
  ...
  if (u) return v; else {
    k.avatarURL = 'http://localhost/samartphone/stock/twitter_egg.png', k.bannerURL = 'https://www.colorhexa.com/cccccc.png';
